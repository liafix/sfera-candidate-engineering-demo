...
Write-Host '=== SFERA Candidate Demo - Phase 5.5 verification ==='
...
Write-Host '[11/11] Phase 5.5 source contract checks'
...
if ($frontendPackage -notmatch '"next"\s*:\s*"16\.3\.1"') {
    throw 'Expected patched Next.js 16.3.1 pin is missing.'
}
if ($frontendPackage -notmatch '"eslint-config-next"\s*:\s*"16\.3\.1"') {
    throw 'Expected eslint-config-next 16.3.1 pin is missing.'
}
...
Write-Host 'PHASE 5.5 BUILD GATE: PASS' -ForegroundColor Green
